#!/usr/bin/env python3
"""``scripts/from_openpi.py`` -- read a checkpoint's ``metadata.pt`` and diff it.
...
This package's equivalent is a ``--robot`` preset, which is a *hand* transcription
of the same facts. This script closes the loop: it reads ``metadata.pt``, prints
the ``apxinf-robo serve`` command it implies, and diffs every field against the
preset the operator named. Then it runs the checkpoint + preset checks from
:mod:`apxinf_robo.preflight` (norm_stats widths, tokenizer, cameras) on top.

It does *not* generate a config file. The counterpart to ``TrainConfig`` is
:mod:`apxinf_robo.embodiments` plus a builder -- code, reviewed once, not
generated per checkpoint. The value here is the assertion, not the transcription.

This lives in ``scripts/`` rather than under ``apxinf-robo`` because it is a
porting-time tool: it answers "how must this checkpoint be served" once, while
the CLI serves what is already decided. It needs the package importable, so run
it from a checkout with ``pip install -e .`` in effect.

Usage::

    python scripts/from_openpi.py --model-dir CKPT_DIR --robot unitree_g1
...
from apxinf_robo.embodiments.base import ROBOT_PRESETS, available_robots, get_robot_preset
from apxinf_robo.preflight import (
    FAIL,
    INFO,
    WARN,
    Finding,
...
def describe_launch(config: dict, robot: str, model_dir: pathlib.Path) -> str:
    """The ``apxinf-robo serve`` line this checkpoint's own metadata implies."""
    preset = get_robot_preset(robot)
    data = config.get("data") or {}
    model = config.get("model") or {}
    flags = [
...
def run(args: argparse.Namespace) -> int:
    """Print the cross-check report. Returns the process exit status."""
