...
# Engine side: the model, the pipeline machinery, the transport. Nothing here
# knows a robot exists -- that asymmetry is half of what these tests assert.
from apxinf import Pi05Policy
from apxinf.policies.base import ComposablePolicy
from apxinf.policies.impls import pi05 as pi05_module
...
from apxinf.processors.transforms import Tokenize, has_key, lookup_key, set_key

# Only the end-to-end serving test needs a real transport. Everything else is
# pure numpy, and that majority is worth running on a machine that has neither
# the client library nor a websocket stack installed.
try:
    ...
    _TRANSPORT_IMPORT_ERROR: str | None = str(exc)
    websocket_client_policy = websocket_server = None
    WebsocketPolicyServer = health_check = None
else:
    _TRANSPORT_IMPORT_ERROR = None

needs_transport = unittest.skipIf(
    _TRANSPORT_IMPORT_ERROR is not None,
    f"needs openpi-client and websockets ({_TRANSPORT_IMPORT_ERROR})",
)

# Robo side: dialects, bodies, presets.
from apxinf_robo import conventions as conventions_module
from apxinf_robo.conventions import (
    CONVENTIONS,
...
from apxinf_robo.embodiments.base import (
    ROBOT_ALIASES,
    ROBOT_PRESETS,
    VIEW_SLOTS,
    Embodiment,
...
from apxinf_robo.embodiments.unitree_g1 import builder as robot_adapter
from apxinf_robo.embodiments.unitree_g1.steps import (
    G1_ROBOT_DIM,
...
class UnitreeG1AdapterTest(unittest.TestCase):
    ...
    def _patched_loader(self, captured: dict, policy):
        def load(model_dir, *, model_type=None, **kwargs):
            # Mirror load_policy's real split: it consumes model_type itself and
        ...
        return mock.patch.object(robot_adapter, "load_policy", load)
...
class ConventionsAreTheirOwnLayerTest(unittest.TestCase):
    ...
    def test_the_g1_steps_no_longer_hold_the_g1s_wire_keys(self) -> None:
        ...
        from apxinf_robo.embodiments.unitree_g1 import steps as g1_steps
    ...
    def test_conventions_import_no_body_and_no_policy_implementation(self) -> None:
        ...
        package = pathlib.Path(conventions_module.__file__).parent
        modules = sorted(package.glob("*.py"))
        self.assertTrue(modules, f"no convention modules found under {package}")
        for module in modules:
            source = module.read_text()
            for forbidden in ("Embodiment", "RobotPreset", "Pi05Policy", "AutoPolicy"):
                with self.subTest(module=module.name, name=forbidden):
...
class NormalizationDtypeTest(unittest.TestCase):
    ...
    def test_the_g1_preset_asks_the_loader_for_float64(self) -> None:
        ...
        with mock.patch.object(robot_adapter, "load_policy", load):
            build_robot_policy("unitree_g1", "/nowhere")
