...
def _require_torch():
    ...
    try:
        ...
        raise ImportError(
            "apxinf_robo.integrations.lerobot needs PyTorch for its tensor boundary; "
            "install it with `pip install apxinf-robo[lerobot]`."
        ) from exc
    return torch
...
def _lerobot_frame_builder():
    ...
    try:
        ...
        raise ImportError(
            "apxinf_robo.integrations.lerobot.build_inference_frame needs lerobot installed "
    ...
    for module_name in ("lerobot.utils.feature_utils", "lerobot.datasets.utils"):
        try:
            ...
            last_error = exc
    raise ImportError(
        "apxinf_robo.integrations.lerobot: could not find lerobot's build_dataset_frame in "
...
class ApxInfPolicy:
    ...
    def from_pretrained(
        ...
        **policy_kwargs: Any,
        ...
        Dispatches through :func:`apxinf_robo.engine.load_policy`, so the
        ...
        policy = load_policy(model_dir, model_type=model_type, **policy_kwargs)
        return cls(
            policy,
