...
Wire keys are supplied by a :class:`~apxinf_robo.conventions.base.Convention`.
...
``with_adapter`` is strict nesting — ``before`` is prepended to the input
pipeline and ``after`` is appended to the output pipeline — so this file cannot
reach into the middle of the engine's chain even by accident. That is the whole
...
from apxinf.policies.base import ComposablePolicy, Policy
from apxinf.processors.base import StepSpec
...
def build_unitree_g1_policy(
    ...
    * **output** — ``[<the model's output steps>, g1_absolute?, g1_encode?]``
    ...
    dialect lives in :data:`apxinf_robo.conventions.UNITREE_G1`, and defaulting
    to it here would rebuild the robot↔dataset coupling one layer up. Pass a
    convention's fields, or let
    :func:`~apxinf_robo.presets.build_robot_policy` do it from the preset
    table::
