...
policy swapped for :class:`apxinf_robo.integrations.lerobot.ApxInfPolicy`.
Everything on the robot side of the loop — ``robots`` / ``cameras`` /
``hw_to_dataset_features`` / ``make_robot_action`` / ``send_action`` — is
lerobot's and stays untouched; only policy construction and the
observation-frame call change.
...
Requires ``apxinf-robo[lerobot]`` (adds torch), the ``apxinf_py`` CUDA binding, a
...
from apxinf_robo.integrations.lerobot import ApxInfPolicy
...
try:
    ...
    raise SystemExit(
        ...
        f"Install lerobot in this environment, or use apxinf's native API "
        f"(python/apxinf/examples/pi05policy_infer.py), which needs neither "
        f"lerobot nor torch."
    ) from exc
